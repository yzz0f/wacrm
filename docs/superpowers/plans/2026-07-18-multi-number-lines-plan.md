...
11. **[Ajustado durante Fase 2]** Widen del índice único `idx_conversations_account_contact` (migración 036, `UNIQUE(account_id, contact_id)`) a `UNIQUE(account_id, contact_id, line_id)` — sin esto, un mismo contacto no puede tener conversaciones separadas por línea (la restricción actual lo impide directamente a nivel de base de datos). Se agregó a la migración 037 en vez de crear una migración aparte, ya que es parte del mismo cambio lógico y la rama aún no estaba pusheada.
...
5. `src/lib/whatsapp/resolve-conversation.ts:59` — **aquí se implementa la decisión de producto #5 del spec**: al crear una conversación nueva manualmente (sin mensaje entrante previo), usar la línea `is_default` de la cuenta; si la cuenta tiene más de una línea, aceptar un `line_id` explícito como override. Mismo patrón de lookup+insert por `(account_id, contact_id)` que tenía el webhook (`:170-171`, `:199-200`) — agregar `line_id` a ambos, igual que se hizo en Fase 2 para `findOrCreateConversation`.
