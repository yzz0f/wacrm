...
export type ContactPlatform = 'whatsapp' | 'instagram';

export interface Contact {
  id: string;
  user_id: string;
  account_id: string;
  /** Present for platform 'whatsapp' contacts; absent for 'instagram'
   *  ones (migration 043 — Instagram identifies contacts by
   *  external_id, not a phone number). Optional on the type — same
   *  convention as other DB-required-with-default fields elsewhere
   *  in this file (e.g. Profile.account_id) — rather than `| null`,
   *  to avoid forcing every existing WhatsApp-only call site to
   *  narrow it. Every pre-Instagram row still has a real phone. */
  phone?: string;
  /** Digits-only form of `phone`, generated by the DB (migration 022)
   *  and unique per account. Read-only. */
  phone_normalized?: string;
  /** 'whatsapp' | 'instagram' (migration 043). Optional on the type
   *  (same reasoning as `phone` above) — absent means 'whatsapp',
   *  the DB default for every pre-Instagram row. */
  platform?: ContactPlatform;
  /** Platform-scoped id for a non-WhatsApp contact — the IGSID for
   *  Instagram. Absent for 'whatsapp' contacts, which use `phone`
   *  instead. Unique per (account_id, platform). */
  external_id?: string;
  name?: string;
  email?: string;
  company?: string;
...
export interface Conversation {
   ...
   *  Exactly one of `line_id` / `instagram_account_id` is set
   *  (migration 043 — a conversation belongs to exactly one channel
   *  account, of either type). */
  line_id?: string | null;
  /** Which Instagram account this conversation came in on. See
   *  InstagramAccount. Migration 043. */
...
/**
 * A connected Instagram business account (migration 043). Same shape
 * as WhatsAppLine, minus the two-phase Meta register/subscribe flow —
 * Instagram Messaging has no equivalent step, so `registered_at` /
 * `subscribed_apps_at` / `last_registration_error` are carried here
 * for schema symmetry but are not populated by any current flow.
 */
